...
Given the Average Precision for each class, we can compute the \tbf{mean Average Precision} across classes. This number will represent the system performance.
...
\begin{figure}[tb]
	...
	\caption{\tbf{Left}: Faster-RCNN integrates the region proposal step in the whole network by creating a Region Proposal Network (RPN). \tbf{Right}: RPN proposes k anchor boxes of different sizes and width/height ratios. For each anchor box, the RPN will detect whether it contains an object (\tit{cls} layer) and refine its shape and position (\tit{reg} layer). Image source: \cite{DBLP:journals/corr/RenHG015}}
	\label{fig:fasterRCNNfull}
\end{figure}

\subsubsection{A note on training}
The Faster-RCNN model has finally 4 loss functions (figure \ref{fig:lossFasterRCNN}). Let's denote the losses in RPN \tit{the lower head}; the losses in the final layer \tit{the upper head} respectively. There exists various training schemes as described in \cite{DBLP:journals/corr/RenHG015}. We will adopt the approximate joint training scheme which is simpler to implement and still gives similar performance to other schemes. The approximate joint training scheme is described as follows: 
...
Based on the speed and robustness of the Faster-RCNN method (figure \ref{fig:rcnnCompair}), I chose Faster-RCNN method to build our object detection system.
...
\end{bmatrix}
\end{align}
where $(u, v)$ are \tit{pixel coordinates on the image}, $(x,y,z)$ represent the object's position \tit{in the camera's frame}, $f_1, f_2$ (in pixel unit) are focal lengths and $c_1, c_2$ (in pixel unit) are coordinates of the center of the image.
